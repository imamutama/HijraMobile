...
import pages.OnBoarding.Register.ReviewDataPage;
...
public class ReviewDataController extends BaseController {
    ...
    public ReviewDataController scrollKC() throws Exception {
        GeneratedUtils.sleep(500);
        try {
            (new TouchAction<>(driver)).press(PointOption.point(450, 1434))
                    .waitAction(WaitOptions.waitOptions(Duration.ofMillis(300)))
                    .moveTo(PointOption.point(454, 833)).release().perform();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return this;
    ...
    public ReviewDataController validation_nextReview() {
